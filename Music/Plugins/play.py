...
@app.on_message(
    command("الاغاني") & ~filters.edited & ~filters.bot & ~filters.private
)
...
async def music_onoff(_, message: Message):
    ...
    if status in ("ON", "on", "On"):
        ...
        await lel.edit(
            f"**✅ بواسطة المطور تم تشغيل الاغاني {message.chat.title}**"
        )
        ...
        if message.chat.id in DISABLED_GROUPS:
            await lel.edit("» **تم تشغيل الموسيقى في البوت بنجاح.**")
            return
...
@Client.on_message(command(["شغل", f"play@{BOT_USERNAME}", "p"]))
...
"""
    ...
    if chat_id in DISABLED_GROUPS:
        return await message.reply_text(
            f"🧑‍💻 **اسف {message.from_user.mention}, قام المطور بايقاف تشغيل الاغاني للبوت**" 
        )
        return
    user_id = message.from_user.id
    chat_title = message.chat.title
...
"""
        ...
        await LOG_CHAT(message, what)
        query = message.text.split(None, 1)[1]
        mystic = await message.reply_text("**❤️‍🔥 يَتمَ اެݪتشغِيݪ اެلانِ...**")
        try:
...
""",
        ...
        await message.reply_photo(
            ...
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=f"""
<b>❤️‍🔥 اެݪاެغنِيةَ:</b> [{title[:25]}]({link})
<b>❤️‍🔥 اެݪمَدَةَ:</b> {duration}
<b>❤️‍🔥 طَݪبَ مَنِ:</b> {checking}
""",
        )
        return await mystic.delete()
...
"""
        ...
        if d["status"] == "downloading":
            ...
            if per > 500:
                if flex[str(bytesx)] == 3:
                    ...
                    print(
                        f"[{videoid}] تحميل {percentage} بسرعة {speed} | ETA: {eta} ثواني"
                    )
            if per > 800:
                if flex[str(bytesx)] == 4:
                    flex[str(bytesx)] += 1
    ...
    if await is_active_chat(chat_id):
        ...
        m = await CallbackQuery.message.reply_photo(
            photo=thumb,
            caption=f"""
<b>❤️‍🔥 تمت إضافة المسارات إلى قائمة الانتظار</b>

<b>❤️‍🔥 اެݪاެغِنِيَةَ:</b>[{title[:25]}]({url})
<b>❤️‍🔥 اެݪمَدَةَ:</b> {duration}
<b>❤️‍🔥 طَݪبَ مَنِ:</b> {checking}
